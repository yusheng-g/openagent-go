...
func (m *Module) CallInit(ctx context.Context, settingsJSON []byte) ([]byte, error) {
	if m.Mod.ExportedFunction("init") == nil { return settingsJSON, nil }
	if len(settingsJSON) == 0 { return callExport(ctx, m.Mod, "init") }
	allocFn := m.Mod.ExportedFunction("alloc")
	if allocFn == nil { return nil, fmt.Errorf("alloc not exported") }
	allocRes, err := allocFn.Call(ctx, uint64(len(settingsJSON)))
	if err != nil || len(allocRes) == 0 { return nil, fmt.Errorf("alloc settings: %w", err) }
	ptr := uint32(allocRes[0])
	m.Mod.Memory().Write(ptr, settingsJSON)
	fn := m.Mod.ExportedFunction("init")
	if fn == nil { return settingsJSON, nil }
	results, err := fn.Call(ctx, uint64(ptr), uint64(len(settingsJSON)))
	if err != nil { return nil, fmt.Errorf("init: %w", err) }
	if len(results) == 0 { return nil, fmt.Errorf("init: no result") }
	return readPacked(m.Mod, results[0])
}

func (m *Module) ReadCommands(ctx context.Context) ([]CommandDef, error) {
	if m.Mod.ExportedFunction("commands") == nil { return nil, nil }
	raw, err := callExport(ctx, m.Mod, "commands")
	if err != nil { return nil, fmt.Errorf("commands: %w", err) }
	var cmds []CommandDef
	if err := json.Unmarshal(raw, &cmds); err != nil { return nil, fmt.Errorf("parse commands: %w", err) }
	for _, cd := range cmds {
...
func RunCommand(ctx context.Context, cmdName string, argsJSON string) (string, error) {
	...
	if !ok { return "", fmt.Errorf("command %q not found", cmdName) }
	mod := v.(api.Module)
	fn := mod.ExportedFunction("run_" + cmdName)
	if fn == nil { return "", fmt.Errorf("command %q has no export run_%s", cmdName, cmdName) }
	allocFn := mod.ExportedFunction("alloc")
	if allocFn == nil { return "", fmt.Errorf("alloc not exported") }
	argsBytes := []byte(argsJSON)
	allocRes, err := allocFn.Call(ctx, uint64(len(argsBytes)))
	if err != nil || len(allocRes) == 0 { return "", fmt.Errorf("alloc: %w", err) }
	ptr := uint32(allocRes[0])
	mod.Memory().Write(ptr, argsBytes)
	results, err := fn.Call(ctx, uint64(ptr), uint64(len(argsBytes)))
	if err != nil { return "", fmt.Errorf("run_%s: %w", cmdName, err) }
	if len(results) == 0 { return "", fmt.Errorf("run_%s: no result", cmdName) }
	data, err := readPacked(mod, results[0])
...
func readCLIMeta(ctx context.Context, mod api.Module) (CLIPluginMeta, error) {
	...
	if err != nil { return CLIPluginMeta{}, err }
	var meta CLIPluginMeta
	if err := json.Unmarshal(raw, &meta); err != nil { return CLIPluginMeta{}, fmt.Errorf("parse metadata: %w", err) }
	return meta, nil
}

func callExport(ctx context.Context, mod api.Module, name string) ([]byte, error) {
	fn := mod.ExportedFunction(name)
	if fn == nil { return nil, fmt.Errorf("export %q not found", name) }
	results, err := fn.Call(ctx)
	if err != nil { return nil, fmt.Errorf("%s: %w", name, err) }
	if len(results) == 0 { return nil, fmt.Errorf("%s: no result", name) }
	return readPacked(mod, results[0])
}

func readPacked(mod api.Module, packed uint64) ([]byte, error) {
	ptr := uint32(packed >> 32)
	length := uint32(packed & 0xFFFFFFFF)
	if ptr == 0 && length == 0 { return nil, nil }
	data, ok := mod.Memory().Read(ptr, length)
	if !ok { return nil, fmt.Errorf("read out of bounds (%d,%d)", ptr, length) }
