...
`AgentInfo.Type` 在 Team 构造 `AgentInfo` 时自动填充（`WithTeamAgent` 设 `AgentInternal`，`AddAgent` 根据 runner 类型自动判断）。类型信息已流通到 Router、Team prompt（显示 `[internal]` / `[external]`）、Plan Planner 和前端。
...
Plan 页面输入 goal → 按钮显示 "Generating plan..." → `plan_thinking` 事件流式展示 LLM 思考过程 → Planner 生成 DAG → 渲染为步骤卡片网格。
